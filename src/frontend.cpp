...
namespace ECT_SLAM
{
    ...
    bool Frontend::MatchAndBuildMap()
    {
        ...
        for (auto m : matches)
        {
            points1.emplace_back(first_frame_->features_[m.queryIdx]->position_.pt.x, first_frame_->features_[m.queryIdx]->position_.pt.y);
            points2.emplace_back(first_frame_->features_[m.trainIdx]->position_.pt.x, first_frame_->features_[m.trainIdx]->position_.pt.y);
        }
        ...
        //!-----------------------Trangulation & Build Map----------------------------
        cv::Mat P1, P2;
        Mat34f P1_e = camera_->K() * first_frame_->RT();
        ...
        SE3 pose_Tcw = first_frame_->Pose().inverse();
        for (int i = 0; i < matches.size(); i++)
        {
            ...
            new_map_point->AddObservation(first_frame_->features_[matches[i].queryIdx]);
            new_map_point->AddObservation(
                current_frame_->features_[matches[i].trainIdx]);

            first_frame_->features_[matches[i].queryIdx]->map_point_ = new_map_point;
            current_frame_->features_[matches[i].trainIdx]->map_point_ = new_map_point;
